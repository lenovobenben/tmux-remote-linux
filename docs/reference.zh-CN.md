...
如果是 Codex 或其他聊天式 agent 使用，agent 应该先在聊天里展示：目标 pane、完整命令、这条命令要做什么的中文解释，以及一个新的随机数字。审批提示应该尽量紧凑，`目标` 放在生产确认行，`说明`、命令、批准数字各自单独成行。字段用 Markdown 粗体和行内代码高亮。不要使用 HTML 标签或内联 CSS，因为某些终端渲染器会把它们原样显示出来。用户只需要回复这个数字，即表示同意执行这一条命令。随后 agent 可以把这个数字和解释作为本次命令的一次性批准参数传给脚本。
