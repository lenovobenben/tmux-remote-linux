...
- **Do not send concurrent input to the same tmux target.** A tmux pane is a serial interactive channel. The agent must wait for one `run.sh`, `send.sh`, or recovery action to finish before starting another on that same target. Different targets may be used in parallel only when the target panes are explicitly distinct.
