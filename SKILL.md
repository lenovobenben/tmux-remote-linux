...
- If you sent `Ctrl-C` while a `run.sh` command was pending, the remote wrapper may have printed its begin marker but not its end marker. A later `run.sh` may then report a stale marker even if the pane has already returned to a normal shell prompt. Use `read.sh` to inspect the pane. If it is visibly back at an ordinary Linux shell prompt, it is acceptable to clear the pane history once with `tmux clear-history -t "${REMOTE_TMUX_TARGET:-remote:0.0}"` and retry `run.sh`. If the pane is not clearly at a shell prompt, do not clear or retry blindly; wait, inspect further, or ask the user.
